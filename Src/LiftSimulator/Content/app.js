(function() {
    ...
    app.controller = app.controller('lift-controller', [
        '$http',
        '$scope',
        function ($http, $scope) {
            ...
            $scope.reset = function () {
                $http.get('/api/v1/simulator/reset/')
                    .success(function (response) {
                        console.log(response);
                        $scope.lifts = response.Context.Lifts;
                        $scope.tick = 0;
            ...
            $scope.getLiftCssClass = function (liftId, level) {
                var lift = _.find($scope.lifts, function (l) { return l.Id && l.Id === liftId; });
                if (lift) {
            ...
            $scope.callLift = function (peopleCount, sourceFloorNumber, targetFloorNumber) {
                ...
                var level = _.find($scope.levels, function (l) { return l.number === sourceFloorNumber });
                if (!level) {
                    return;
                }
